...
- [Kerne Protocol](https://x402-gateway-omega.vercel.app/reserves) - Proof-of-reserves and signed ERC-20 reads for AI agents. GET /reserves ($0.25) returns Kerne's hourly EOA-signed proof-of-reserves attestation for its kUSD synthetic dollar (PSM backing ratio, aggregate USD solvency, all bound to an EIP-191 signature you can verify yourself); POST /instant-read ($29) returns an EIP-191 signed factual read of any ERC-20. USDC on Base via the Coinbase CDP facilitator, x402 v2 with Bazaar discovery on every 402. The same signed reserves blob is served free to humans at https://kerne.fi/api/por/signed, so the paid route is the metered agent-callable delivery, not privileged data. Every response independently verifiable (EIP-191). ([Home](https://kerne.fi) | [OpenAPI](https://x402-gateway-omega.vercel.app/openapi.json))
