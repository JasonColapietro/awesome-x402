...
- [venture-mpp](https://venture-mpp.vercel.app) - HTTP-402 paid API for AI agents via MPP (Tempo's machine-payments protocol, the 402 pattern x402 uses, on Tempo mainnet): AI image generation (`POST /image`, $0.10) plus Tempo token metadata and balance reads ($0.01 each), settled in USDC.e. No API keys, no signup; free `/health`. OpenAPI with x-payment-info offers at [/openapi.json](https://venture-mpp.vercel.app/openapi.json).
