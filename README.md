...
- [Lazaretto](https://lazaretto.dev) - Pre-install security verification for AI agent skills, tools, and packages: known-bad indicator match (abuse.ch feeds, refreshed daily) plus a deterministic scan for credential theft, exfiltration, obfuscation, prompt injection, and install-time droppers, with evidence, no LLM in the serving path. Verdicts are SHA-256-bound so an agent can re-verify what landed on disk. Free known-bad lookup; paid full scan $0.03 USDC on Base (x402) or prepaid credit bundles ($3=100 scans). MCP server + ClawHub skill. ([OpenAPI](https://lazaretto.dev/openapi.json) | [Discovery](https://lazaretto.dev/.well-known/x402) | [Agent Card](https://lazaretto.dev/.well-known/agent-card.json) | [MCP](https://github.com/jamesdfinance-dev/lazaretto-mcp))
