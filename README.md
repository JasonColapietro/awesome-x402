...
- [Suede](https://suedeai.ai) — three public x402-paid media APIs on Base USDC: full-song generation (`POST /create-music`, $0.50), short video (`POST /agent/video`, $4.99), and still image (`POST /agent/image`, $0.15). The canonical live inventory is published at [/.well-known/x402](https://app.suedeai.ai/.well-known/x402) and [/.well-known/x402.json](https://app.suedeai.ai/.well-known/x402.json) — both aliases serve the same three resources; each route returns its current amount in an unpaid `402 Payment Required` challenge. New integrations send the payment payload in `PAYMENT-SIGNATURE`; legacy `X-PAYMENT` remains accepted during migration. A `PAYMENT-SIGNATURE` call runs asynchronously by default — all three routes answer `202 Accepted` with a `pollUrl` rather than the finished asset, and polling it is public and needs no second payment. Older `/v1/*` musician-tool profiles and `/agent/generate` are not current public offerings. ([Reference](https://x402.suedeai.ai) | [OpenAPI](https://app.suedeai.ai/openapi.json) | [Agent card](https://app.suedeai.ai/.well-known/agent-card.json))
- [Suede Agent Studio](https://agents.suedeai.ai) - Visual builder for agent flows that publish as pay-per-call x402 endpoints, with settled calls routing USDC to your wallet on Base. Build in a guided flow, on the canvas, or in TypeScript pushed from the terminal.
- [Agentix](https://agentix.suedeai.ai) - Earnings tracker for x402 agents already launched: total USDC settled on Base, call volume, and per-agent performance on one screen. Companion to Suede Agent Studio, which is where the agents are built.
- [Strumly](https://strumly.suedeai.ai/agents) - Music oracle for agents: chord knowledge, song analysis, chord-progression transcription, and practice plans across four x402 endpoints, $0.99-$4.99 USDC per call on Base. Also accepts Stripe per-call or a $9.99 24h day pass, with 5 free calls per IP per UTC day. ([/.well-known/x402](https://strumly.suedeai.ai/.well-known/x402) | [OpenAPI](https://strumly.suedeai.ai/openapi.json) | [llms.txt](https://strumly.suedeai.ai/llms.txt))
...
- Discovery: `https://app.suedeai.ai/.well-known/x402.json` and `https://app.suedeai.ai/.well-known/x402` — both publish the same three resources
- OpenAPI: `https://app.suedeai.ai/openapi.json`
- Settlement: USDC on Base, asset `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`
- Public endpoint reference (three offerings, current pricing, x402 flow): [suede-x402](https://github.com/JasonColapietro/suede-x402)
