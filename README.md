...
### Data & Social APIs
- [afriref](https://afriref.dev) - Pan-African government reference data for AI agents: central-bank policy rates, VAT, minimum wages and public holidays for South Africa, Nigeria and Kenya — every value cited to its official gazette or central-bank source with effective dates and staleness flags. $0.005 USDC per series call, $0.02 full snapshot (x402 v2, CDP-facilitated on Base, Bazaar discovery); free catalog and public-holiday endpoints; HMAC-signed change-alert webhooks. No API keys, no signup. ([Discovery](https://afriref.dev/.well-known/x402.json)) ([llms.txt](https://afriref.dev/llms.txt))
