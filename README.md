...
- [Suede](https://app.suedeai.ai) — x402-paid AI music, video, and image generation API. Three paid endpoints: `/create-music` ($0.20 USDC) for full-song generation, `/agent/video` ($1.50 USDC) for short video generation, and `/agent/image` ($0.05 USDC) for image generation. Returns live `402 Payment Required` with x402 payment requirements. Discovery via [/.well-known/x402](https://app.suedeai.ai/.well-known/x402), [/.well-known/x402.json](https://app.suedeai.ai/.well-known/x402.json), and [/.well-known/agent-card.json](https://app.suedeai.ai/.well-known/agent-card.json). USDC on Base. ([Live endpoint catalog](https://app.suedeai.ai/.well-known/x402.json))
