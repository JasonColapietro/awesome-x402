...
- [agentdata-nl](https://agentdata-api.sander-van-aard.workers.dev) - Dutch company screening for AI agents, straight from official registers: EU VAT validation (VIES, $0.002), Dutch insolvency check against the judiciary's Central Insolvency Register ($0.15), and one-call supplier screening combining both with a legal-name cross-check ($0.18). All-or-nothing billing — you are only charged when every register was successfully consulted. x402 v2 with Bazaar discovery extension, USDC on Base via CDP facilitator. No API keys, no signup. ([MCP](https://agentdata-api.sander-van-aard.workers.dev/mcp)) ([OpenAPI](https://github.com/DarthSander/data_agent/blob/main/openapi.yaml)) ([GitHub](https://github.com/DarthSander/data_agent))
