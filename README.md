...
### Data & Social APIs
- [WebIntel](https://x402.omardev.xyz) - EU/NL-focused web intelligence for AI agents on Base. 7 pay-per-call endpoints: page-to-markdown ($0.01), page metadata ($0.005), sitemap discovery ($0.01), tech-stack detection ($0.05), DNS & email-provider intel ($0.02), EU company enrichment with live VIES VAT validation plus Dutch KvK/BTW ($0.10), and a full domain dossier ($0.25). USDC on Base via Coinbase CDP facilitator, Bazaar discovery extension on every 402, no API keys or signup. ([Catalog](https://x402.omardev.xyz/))
