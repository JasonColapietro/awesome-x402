...
- [Suede](https://suedeai.ai) — x402-paid AI music, video, image, and musician-tool API. 20 confirmed-live paid endpoints on Base USDC, $0.003–$1.50 per call: full-song generation (`/create-music`, $0.20), short video (`/agent/video`, $1.50), image (`/agent/image`, $0.05), 12 musician tools (stems, mastering, lyrics, vocal isolation, and more), rights lookup, prompt/audio analysis, and on-chain Q&A, plus a 3-tier credit-purchase route. Every endpoint returns a live `402 Payment Required` challenge with x402 payment requirements. Public discovery ([/.well-known/x402](https://app.suedeai.ai/.well-known/x402), [/.well-known/x402.json](https://app.suedeai.ai/.well-known/x402.json), [/.well-known/agent-card.json](https://app.suedeai.ai/.well-known/agent-card.json)) is deliberately scoped to the 3 media-generation endpoints only — credits and the other 17 tool/utility endpoints are live but direct-call only; the full endpoint table lives in [suede-x402-acp](https://github.com/JasonColapietro/suede-x402-acp).
...
- Full endpoint reference (20 endpoints, current pricing, verification log): [suede-x402-acp](https://github.com/JasonColapietro/suede-x402-acp)
