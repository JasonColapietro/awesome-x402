...
- [jurat](https://jurat.dev) - Evidence infrastructure for AI agents. Notarized snapshots of any public URL (ed25519-signed attestations, archived bytes, daily merkle roots, verification free forever), sports finals with proof attached across 24 leagues incl. rugby and cricket, change watchers and scheduled wakeups for ephemeral agents, and x402 pre-flight: 18 conformance checks on any other x402 endpoint before you pay it, including whether the price it advertises in its openapi.json matches what its live 402 actually charges. 9 paid endpoints at $0.005-$0.10 USDC on Base via the CDP facilitator, 13 free forever. Invalid input is rejected with a free 400, so a buyer never pays for a malformed request. No API keys, no signup. ([Catalog](https://jurat.dev/catalog)) ([OpenAPI](https://jurat.dev/openapi.json)) ([llms.txt](https://jurat.dev/llms.txt)) ([Merchant page](https://tryponcho.com/m/jurat.dev))
