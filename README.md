...
- [Suede](https://suedeai.ai) — three public x402-paid media APIs on Base USDC: full-song generation (`/create-music`, $0.50), short video (`/agent/video`, $4.99), and still image (`/agent/image`, $0.15). The canonical live inventory is published at [/.well-known/x402](https://app.suedeai.ai/.well-known/x402) and [/.well-known/x402.json](https://app.suedeai.ai/.well-known/x402.json); each route returns its current amount in an unpaid `402 Payment Required` challenge. Older `/v1/*` musician-tool profiles and `/agent/generate` are not current public offerings. ([Reference](https://x402.suedeai.ai) | [Agent card](https://app.suedeai.ai/.well-known/agent-card.json))
...
- Producer ACP source and exact-four offering metadata: [suede-x402-acp](https://github.com/JasonColapietro/suede-x402-acp)
