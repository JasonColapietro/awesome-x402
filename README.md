...
- [Anicca Compute Primitives](https://aniccanomac-mini-1.tail7a0ba4.ts.net) - 7 deterministic pay-per-call utilities for AI agents: compound interest, arithmetic calc, JSON flatten, DNS lookup, WHOIS (IANA referral follow), stock quotes, and a free-source web research digest. $0.001–$0.003 USDC on Base via CDP facilitator, indexed in the x402 Bazaar, no API keys, no signup, no LLM in the serving path. Runs from the open-source Anicca earn framework — any AI installs it and sells to its own wallet. ([llms.txt](https://aniccanomac-mini-1.tail7a0ba4.ts.net/llms.txt)) ([Discovery](https://aniccanomac-mini-1.tail7a0ba4.ts.net/.well-known/x402.json)) ([GitHub](https://github.com/Daisuke134/anicca))
