...
- [Token Intel](https://token-intel-x402.echolonius.deno.net) - Solana token due-diligence for AI agents: one call fuses on-chain safety signals (mint/freeze authorities, holder concentration, dev holdings) with market data from two independent sources, cross-checks the sources for disagreement red flags, and returns a synthesized 0–100 risk verdict. $0.02 USDC per call on Base via the keyless PayAI facilitator — no API keys, no signup. Free demo route to verify the pipeline before paying. ([Discovery](https://token-intel-x402.echolonius.deno.net/.well-known/x402)) ([OpenAPI](https://token-intel-x402.echolonius.deno.net/openapi.json)) ([Demo](https://token-intel-x402.echolonius.deno.net/api/token-intel/demo))
