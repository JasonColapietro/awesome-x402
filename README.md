...
### Data & Social APIs
- [afriref / latamref / asiaref / euroref / usaref / ausref](https://afriref.dev) - Verified government reference data for 44 countries across six continent-scoped brands on one engine: central-bank policy rates, VAT, minimum wages, CPI inflation, corporate & personal income-tax brackets and public holidays — 308 series, every value cited to its official gazette, central-bank or statistics-office source with effective dates and staleness flags. Africa 16 (incl. all 7 WAEMU members sharing one reconciled BCEAO rate), Latin America 16, Asia 6, Europe 2, North America 2, Australasia 2. Honest structural nulls where no figure exists rather than a fabricated one — the US has no federal VAT, Singapore runs an S$NEER band instead of a policy rate, Argentina abolished its policy rate. From $0.005 USDC per series call (x402 v2, CDP-facilitated on Base, Bazaar discovery); free catalog and public-holiday endpoints; HMAC-signed change-alert webhooks; computed answers (VAT breakdown, minimum-wage compliance, working days, income tax, gross-to-net payroll); and cross-border VAT determination for digital-services sellers (EU, UK, South Africa), each rule cited to the statute. No API keys, no signup. ([Discovery](https://afriref.dev/.well-known/x402.json)) ([llms.txt](https://afriref.dev/llms.txt))
